...
const setBoardField = (
  ...
  column: number,
  value: number,
  tempBoard: SudokuFieldProps[]
) => {
  const field = tempBoard.find((f) => {
...
const solveOneStep = (tempBoard: SudokuFieldProps[]): boolean => {
  let result = false;
  const emptyFields = tempBoard.filter((f) => {
    return f.value === null;
  });
  ...
  emptyFields.forEach((emptyField) => {
    const row = tempBoard
      .filter((f) => {
        return f.row === emptyField.row && f.value;
      })
      .map((m) => {
        return m.value;
      });
    const column = tempBoard
      .filter((f) => {
        return f.column === emptyField.column && f.value;
      })
      .map((m) => {
        return m.value;
      });
    const block = tempBoard
      .filter((f) => {
        return f.block === emptyField.block && f.value;
      })
      .map((m) => {
        return m.value;
      });
    ...
    emptyField.possibleValues = possibleValues.filter((f) => {
      return !usedValues.includes(f);
    });
  });

  const knownFields = tempBoard.filter((f) => {
    return f.possibleValues.length === 1;
  });

  const errorFields = tempBoard.filter((f) => {
    return f.possibleValues.length === 0 && f.value === null;
  });
  ...
  for (let column = 0; column < 9; column++) {
    for (let row = 0; row < 9; row++) {
      board.push({
        row: row,
        column: column,
        block: getBlockNumber(row, column),
        value: null,
        possibleValues: [],
      });
    }
